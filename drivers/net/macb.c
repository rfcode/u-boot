...
int macb_miiphy_read(char *name, unsigned char addr, unsigned char reg, unsigned short *value)
{
	struct eth_device *netdev;
	...
	debug("Asked to read register 0x%x in %s @ 0x%x\n", reg, name, addr);
	netdev = eth_get_dev_by_name(name);
	/*if (netdev == NULL)
	{
...
int macb_miiphy_write(char *name, unsigned char addr, unsigned char reg, unsigned short value)
{
	struct eth_device *netdev;
	struct macb_device *macb;
	unsigned long netctl;
	...
	debug("Asked to write 0x%x to register 0x%x in %s @ 0x%x\n", value, reg, name, addr);
	netdev = eth_get_dev_by_name(name);
	/*if (netdev == NULL)
	{
		printf("Error: Device '%s' not found\n", name);
...
int macb_eth_initialize(int id, void *regs, unsigned int phy_addr)
{
	...
	/* Abort if no PHY present at the given phy_addr */
	if (macb_mdio_read(macb, 0) == 0xFFFF) {
		free(macb->tx_ring_dma);
		free(macb->rx_ring_dma);
		free(macb->rx_buffer_dma);
		free(macb);
		return -1;
	}
#endif	
 ...
 #ifdef CONFIG_RFCTHUNDERBOLT
/* Just configure the HW address without trying to enable the interface */
void macb_set_ethaddr(void)
{
	struct eth_device *netdev = eth_get_dev();
	struct macb_device *macb = to_macb(netdev);
 	u32 hwaddr_bottom;
	u16 hwaddr_top;
