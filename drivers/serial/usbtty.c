...
#define ACM_TYPE 0
#define GSERIAL_TYPE 1
#define ACM_TX_ENDPOINT 3
#define ACM_RX_ENDPOINT 2
#define ACM_STATUS_ENDPOINT 1
...
static unsigned short status_endpoint = 0;
static unsigned short interface_count = 0;
static struct usb_string_descriptor *usbtty_string_table[STR_COUNT];
static short ttytype = GSERIAL_TYPE;
...
static u8 wstrAltManufacturer[2 + 2*(sizeof(CONFIG_USBD_ALT_MANUFACTURER)-1)];
static u8 wstrAltProduct[2 + 2*(sizeof(CONFIG_USBD_ALT_PRODUCT_NAME)-1)];
#endif
...
#ifdef CONFIG_RFCTHUNDERBOLT
	.iSerialNumber =	0,	// for now atleast
#else
...
static struct acm_serial_status serial_state_response={
		.bmRequestType	=	USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE,
		.bNotification	=	ACM_SERIAL_STATE,
		.wValue		=	cpu_to_le16(0),
		.wIndex		=	cpu_to_le16(0),
		.wLength	=	cpu_to_le16(2),
		.bmState	=	cpu_to_le16(0x0002)	// DSR=1
};
...
#ifdef CONFIG_RFCTHUNDERBOLT
	/* Poll since putc() sometimes outputs a progress indicator */
	usbtty_poll ();
#else
...
int drv_usbtty_init (void)
{
	...
	if (!strcmp(tt,"cdc_acm"))
		ttytype = ACM_TYPE;
	else	ttytype = GSERIAL_TYPE;
	usbtty_init_terminal_type(ttytype);
...
#ifdef CONFIG_RFCTHUNDERBOLT
	/* If the boardtype is non-zero, use generic RFCode USB strings	*/
	if (get_board_rev() != 0)
	{
		string = (struct usb_string_descriptor *) wstrAltManufacturer;
		string->bLength = sizeof(wstrAltManufacturer);
		string->bDescriptorType = USB_DT_STRING;
		str2wide (CONFIG_USBD_ALT_MANUFACTURER, string->wData);
		usbtty_string_table[STR_MANUFACTURER]=string;

		string = (struct usb_string_descriptor *) wstrAltProduct;
		string->bLength = sizeof(wstrAltProduct);
		string->bDescriptorType = USB_DT_STRING;
		str2wide (CONFIG_USBD_ALT_PRODUCT_NAME, string->wData);
		usbtty_string_table[STR_PRODUCT]=string;
	}
	else /* Original M250 USB strings (boardtype = 0)		*/
	{
...
static void usbtty_init_endpoints (void)
{
	...
	for (i = 1; i <= NUM_ENDPOINTS; i++) {
		...
		/* Cancel any pending tx URB since the ep was reset */
		if (endpoint_instance[i].endpoint_address & USB_DIR_IN) {
			endpoint_instance[i].tx_urb->actual_length = 0;
			endpoint_instance[i].sent = 0;
			endpoint_instance[i].last = 0;
		}
	}
}
...
static void usbtty_init_terminal_type(short type)
{
	switch(type){
		...
		case 0:
...
#ifdef CONFIG_RFCTHUNDERBOLT
			/* If this is not an original M250 board id	*/
			if (get_board_rev() != 0)
			{
				/* Use the alternate vendor id, product id, bcd device */
				device_descriptor.idVendor =
					cpu_to_le16(CONFIG_USBD_ALT_VENDORID);
				device_descriptor.idProduct =
					cpu_to_le16(CONFIG_USBD_ALT_PRODUCTID_CDCACM);
				device_descriptor.bcdDevice =
					cpu_to_le16(CONFIG_USBD_ALT_BCD_DEVICE);
				/* M240 (Econobox) is bus powered	*/
				if (get_board_rev() == 1)
				{
					acm_configuration_descriptors[0].configuration_desc.bmAttributes &= ~BMATTRIBUTE_SELF_POWERED;
				}
			}
#endif
			...
			status_endpoint = ACM_STATUS_ENDPOINT;
...
static int write_buffer (circbuf_t * buf)
{
	 ...
	 */
	if((endpoint->sent + endpoint->last) < current_urb->actual_length){
#ifdef CONFIG_RFCTHUNDERBOLT
		unsigned timeout=500000;
		while (endpoint->last) {
			udc_irq();
...
/* Send a canned serial state response indicating DSR is set */
static int send_serial_state (void)
{
	struct usb_endpoint_instance *endpoint =
		&endpoint_instance[status_endpoint];

	/* If all previous data has been sent */
	if (endpoint->tx_urb->actual_length == 0) {
		/*  Queue a serial state response message */
		endpoint->tx_urb->buffer = (u8*)&serial_state_response;
		endpoint->tx_urb->actual_length = sizeof(serial_state_response);
				  ...
				  usb_device_event_t event, int data)
{
	...
	case DEVICE_CONFIGURED:
		...
		if (ttytype == ACM_TYPE) {
			/* queue a serial state response urb */
			if (send_serial_state()) {
				TTYERR("unable to send serial state notification\n");
			}
		}
		break;
...
int usbtty_cdc_setup(struct usb_device_request *request, struct urb *urb)
{
	switch (request->bRequest){
		...
		case ACM_SET_CONTROL_LINE_STATE:	/* Implies DTE ready */
			TTYDBG("control line state: 0x%x\n",
				le16_to_cpu(urb->device_request.wValue));
			break;
