...
int devices_init (void)
{
	...
	drv_logbuff_init ();
#endif
#if defined(CONFIG_RFCTHUNDERBOLT) && defined(CONFIG_USB_TTY)
	{
		/* Env variable that will force us to use USB */
		if (getenv("forceusb"))
			usbtty_console_in_use = 1;
		/* Run-time conditions to enable USB tty */
		if (usbtty_console_in_use) {
			/* USB tty first so it has priority */
			drv_usbtty_init ();
		}
		drv_system_init ();
	}
