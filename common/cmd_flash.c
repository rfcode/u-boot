...
#include <jffs2/jffs2.h>
/* flash_jffs2_cleanmark
 * This function scans for blank sectors and adds a JFFS2 cleanmarker node
 * to those that need it. This allows the kernel to mount a fresh image more
 * quickly. This routine does not verify that the entire sector is blank.
 */
int flash_jffs2_cleanmark (ulong addr_first, ulong addr_last)
{
	flash_info_t *info;
	...
	if (planned && (rcode == 0)) {
		struct jffs2_unknown_node cleanmarker = {
			...
			.totlen = sizeof(cleanmarker),
		};
		cleanmarker.hdr_crc = crc32_no_comp(0, 
		...
		for (bank=0,info = &flash_info[0]; bank < CFG_MAX_FLASH_BANKS; ++bank, ++info) {
			...
			debug ("Checking sectors %d..%d in bank %ld\n",
				s_first[bank], s_last[bank], bank+1);
				
			for (i=s_first[bank]; i<=s_last[bank]; ++i) {
				node = (struct jffs2_unknown_node *)info->start[i];
				
				debug("Node @ %p: magic=0x%04X type=0x%04X totlen=0x%08X crc=0x%08X\n",
					node, node->magic, node->nodetype, node->totlen, node->hdr_crc);
				/* If any fields are not blank, skip this sector */
				if (	(node->magic != 0xFFFF) ||
					(node->nodetype != 0xFFFF) ||
					(node->totlen != 0xFFFFFFFF) ||
					(node->hdr_crc != 0xFFFFFFFF)) {
					continue;
				}
				/* Write the cleanmarker node to this sector */
				rcode = flash_write((char *)&cleanmarker, 
					info->start[i], sizeof(cleanmarker));
				if (rcode) {
		...
		printf ("Added cleanmarker to %d sectors\n", marked);
	} else if (rcode == 0) {
...
int do_jffs2_cleanmark (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong addr_first, addr_last;
	...
	return flash_jffs2_cleanmark (addr_first, addr_last);
}

U_BOOT_CMD(
	jclean,    3,    1,    do_jffs2_cleanmark,
	"jclean  - write JFFS2 clean markers in blank sectors\n",
	"start end\n"
	"    - write markers in sectors from addr 'start' to addr 'end'\n"
	"jffs2clean start +len\n"
	"    - write markers in sectors from addr 'start' to the end of\n"
	"      sector w/addr 'start'+'len'-1\n"
);
#endif /* CONFIG_RFCTHUNDERBOLT */
